...
public class SaleProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (match) {
            case SALE:
                ...
                cursor = database.query(SaleContract.SaleEntry.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder);
                break;
            case SALE_ID:
    ...
    public Uri insert(Uri uri, ContentValues contentValues) {
        ...
        switch (match) {
            case SALE:
                return registerSale(uri, contentValues);
            default:
                throw new IllegalArgumentException("Insertion is not supported for " + uri);
        }
    }

    private Uri registerSale(Uri uri, ContentValues values) {
    ...
    private int updateSale(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(SaleEntry.COLUMN_ITEM_DESCRIPTION)) {
            Integer gender = values.getAsInteger(SaleEntry.COLUMN_ITEM_DESCRIPTION);
            if (gender == null) {
                throw new IllegalArgumentException("Sale requires a valid description");
            }
        }
